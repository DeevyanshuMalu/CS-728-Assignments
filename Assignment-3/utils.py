...
class PromptUtils:
    def __init__(self, tokenizer, doc_ids, dict_all_docs):
        ...
        self.prompt_prefix_length = len(
            tokenizer(self.prompt_prefix, add_special_tokens=False).input_ids
        )
        self.prompt_suffix_length = len(
            tokenizer(self.prompt_suffix, add_special_tokens=False).input_ids
        )

        self.doc_text = (
            lambda idx, doc_name, doc_info: f"tool_id: {doc_name}\ntool description: {doc_info}"
        )
        self.add_text1 = (
            f"Now, please output ONLY the correct tool_id for the query below."
        )
        ...
        self.add_text1_length = len(
            tokenizer(self.add_text1, add_special_tokens=False).input_ids
        )
    ...
    def create_doc_pool_string(self, shuffled_keys, all_docs):
        ...
        for ix, key in enumerate(shuffled_keys):
            ...
            doc_text = self.doc_text(
                idx=self.dict_doc_name_id[key] + 1, doc_name=key, doc_info=value
            ).strip()
            doc_text_len = len(
                self.tokenizer(doc_text, add_special_tokens=False).input_ids
            )
            text += doc_text
...
def get_queries_and_items_check():
    ...
    for idx in range(len(df)):
        row = df.iloc[idx]
        queries.append(
            {"text": row["Query"], "gold_tool_name": row["Tool"], "qid": idx}
        )
    ...
    with open("data/test_queries.json", "w") as f:
        json.dump(queries_filtered[:5000], f)
    with open("data/train_queries.json", "w") as f:
        json.dump(queries_filtered[5000:6500], f)
    with open("data/tools.json", "w") as f:
        json.dump(dbs_filtered, f)
...
def get_queries_and_items():
    ...
    with open("data/train_queries.json", "r") as f:
