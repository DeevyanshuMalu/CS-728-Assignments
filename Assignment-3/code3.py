...
def get_toolwise_attention_scores(query_span, item_spans, attentions):
    toolwise_scores = torch.zeros(
        (len(item_spans), len(attentions), attentions[0].size(1)),
...
def attention_norm_on_tools_score(toolwise_scores, gold_tool_id):
    score = toolwise_scores[gold_tool_id] / (
        toolwise_scores.sum(dim=0) + 1e-8
...
def attention_rank_tools_score(toolwise_scores, gold_tool_id):
    ...
    sorted_ids = torch.argsort(
        toolwise_scores, dim=0, descending=True
    )  # shape: (num_tools, num_layers, num_heads)
    gold_tool_ranks = torch.argsort(sorted_ids, dim=0)[
        gold_tool_id
    ]  # shape: (num_layers, num_heads)
...
def select_retrieval_heads(
    ...
    for qix in tqdm(range(len(train_queries))):
        ...
        with torch.no_grad():
            attentions = model(
                **inputs
            ).attentions  # list of (batch_size, num_heads, seq_len, seq_len) for each layer
        ...
        toolwise_scores = get_toolwise_attention_scores(
            query_span, item_spans, attentions
        )  # shape: (num_tools, num_layers, num_heads)

        if selection_method == "norm":
            head_scores_query_to_tools = attention_norm_on_tools_score(
                toolwise_scores, gold_tool_id
            )  # shape: (num_layers, num_heads)
        elif selection_method == "rank":
            head_scores_query_to_tools = attention_rank_tools_score(
                toolwise_scores, gold_tool_id
            )  # shape: (num_layers, num_heads)
        else:
